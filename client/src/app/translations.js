export const DEFAULT_LANGUAGE = 'tr'

export const translations = {
  tr: {
    header: {
      backToGame: 'Oyuna Dön',
      account: 'Hesap',
      language: 'Dil',
      settings: 'Ayarlar',
      accountSettings: 'Hesap Ayarları',
      logOut: 'Çıkış Yap',
      english: 'English',
      turkish: 'Türkçe',
      openProfile: 'Profili Aç',
      goHome: 'Ana Sayfaya Dön',
      notifications: 'Bildirimler',
      noNewNotifications: 'Yeni bildirim yok',
      wantsToBeFriends: 'seninle arkadaş olmak istiyor!',
      accept: 'Kabul Et',
      reject: 'Reddet',
      verifyEmailWarning: 'Lütfen e-posta adresinizi doğrulayın. Hesabınız 24 saat içinde doğrulanmazsa devre dışı bırakılacaktır.',
    },
    common: {
      close: 'Kapat',
      rated: 'Dereceli',
      unrated: 'Derecesiz',
      cancel: 'Vazgeç',
      loadingMatch: 'Maç yükleniyor',
      fetchingLatestState: 'En son oyun durumu getiriliyor...',
      gameUnavailable: 'Oyun kullanılamıyor',
      noSuchPlayer: 'Böyle bir oyuncu yok',
      draw: 'Berabere',
      or: 'VEYA',
      action: 'İşlem',
      watch: 'İzle',
      resign: 'Çekil',
      globalNavigation: 'Genel gezinme',
      couldNotLoadMatch: 'Maç yüklenemedi.',
    },
    auth: {
      usernameOrEmail: 'Kullanıcı adı veya e-posta',
      username: 'Kullanıcı adı',
      email: 'E-posta',
      password: 'Şifre',
      confirmPassword: 'Şifreyi Onayla',
      rememberMe: 'Beni hatırla',
      loggingIn: 'Giriş yapılıyor...',
      logIn: 'Giriş Yap',
      signUp: 'Kayıt Ol',
      signingUp: 'Kayıt olunuyor...',
      playAsGuest: 'Misafir Olarak Oyna',
      continueWithGoogle: 'Google ile Devam Et',
      backToLogin: 'Girişe Dön',
      loginMissing: 'Lütfen e-posta/kullanıcı adı ve şifrenizi girin.',
      loginFailed: 'Giriş başarısız. Lütfen tekrar deneyin.',
      registerMissing: 'Lütfen kullanıcı adı, e-posta ve şifre alanlarını doldurun.',
      usernameRules:
        'Kullanıcı adı 3-15 karakter olmalı ve yalnızca harf, sayı, alt çizgi veya tire içermelidir.',
      passwordLength: 'Şifre {min}-{max} karakter uzunluğunda olmalıdır.',
      passwordsNoMatch: 'Şifreler eşleşmiyor.',
      registrationFailed: 'Kayıt başarısız. Lütfen tekrar deneyin.',
      resendVerification: 'Doğrulama E-postasını Yeniden Gönder',
      emailVerification: 'E-posta Doğrulama',
      verifyingEmail: 'E-postanız doğrulanıyor, lütfen bekleyin...',
      emailVerifiedSuccess: 'E-posta başarıyla doğrulandı!',
      emailVerifiedError: 'Doğrulama başarısız oldu. Bağlantı süresi dolmuş olabilir.',
      noToken: 'Doğrulama bağlantısı geçersiz.',
      verificationResent: 'Doğrulama e-postası başarıyla yeniden gönderildi.',
      verificationResendFailed: 'Doğrulama e-postası yeniden gönderilemedi.',
      forgotPasswordDesc: 'Şifrenizi sıfırlamanız için bir bağlantı göndereceğiz. Lütfen e-posta adresinizi girin.',
      forgotPasswordSuccess: 'E-postanız kayıtlıysa, şifre sıfırlama bağlantısı gönderilmiştir.',
      forgotPasswordError: 'Şifre sıfırlama e-postası gönderilemedi.',
      sendResetLink: 'Sıfırlama Bağlantısı Gönder',
      sendingResetLink: 'Gönderiliyor...',
      invalidResetToken: 'Geçersiz veya eksik sıfırlama kodu.',
      bothPasswordsRequired: 'Her iki şifre alanı da doldurulmalıdır.',
      passwordResetSuccess: 'Şifreniz başarıyla sıfırlandı.',
      passwordResetError: 'Şifre sıfırlanamadı.',
      createNewPassword: 'Yeni Şifre Oluştur',
      newPassword: 'Yeni Şifre',
      resettingPassword: 'Sıfırlanıyor...',
      resetPassword: 'Şifreyi Sıfırla',
      redirecting: 'Yönlendiriliyor...',
      forgotPasswordLink: 'Şifremi Unuttum?',
      completeProfileTitle: 'Profilini Tamamla',
      completeProfileDesc: 'Lütfen kendinize bir kullanıcı adı seçin. Bunu daha sonra değiştirebilirsiniz.',
      completeProfileSubmit: 'Kaydet ve Başla',
      completingProfile: 'Kaydediliyor...',
    },
    game: {
      youWin: 'Kazandınız',
      youLose: 'Kaybettiniz',
      matchFinished: 'Maç bitti',
      matchResult: 'Maç sonucu',
      viewingSelectedPosition: 'Seçili konum görüntüleniyor.',
      viewingLatestPosition: 'Son konum görüntüleniyor.',
      startPosition: 'Başlangıç konumu görüntüleniyor.',
      matchEndsInDraw: 'Maç berabere biter.',
      wins: '{name} kazandı.',
      toMove: '{name} oynayacak',
      visualStones: 'Görsel Taşlar',
      moveAnimation: 'Hamle Animasyonu',
      unavailable: 'Yok',
      on: 'Açık',
      off: 'Kapalı',
      moveNumber: 'Hamle',
      restartMatch: 'Maçı yeniden başlat',
      gameUnavailableDescription: 'Bu oyun mevcut oturumda kullanılamıyor.',
      winsOnTime: '{name} süreyle kazandı.',
      sowingStones: '{name} taşları dağıtıyor...',
      collectsStonesAndWins: '{name} daha fazla taş topladı ve kazandı.',
      playsAgain: '{name} hazinesine ulaştı ve tekrar oynuyor.',
      capturedStones: '{activeName} {captured} taş ele geçirdi. Sıradaki oyuncu {nextName}.',
      winsByResignation: '{name} çekilme ile kazandı.',
      botOpponent: 'Bot rakip',
    },
    home: {
      welcome: 'Hoş geldin {name}!',
      guestHeroTitle: 'Mangala\'yı çevrimiçi oyna.',
      learnTrain: 'Öğren ve Çalış',
      resumeCurrentGame: 'Mevcut Oyuna Dön',
      play: 'Oyna',
      practiceBoard: 'Alıştırma Tahtası',
      lobbySidePanel: 'Lobi yan paneli',
      availablePlayers: 'Uygun Oyuncular',
      noAvailablePlayers: 'Şu anda uygun oyuncu yok.',
      leaderboard: 'Liderlik Tablosu',
      showMore: 'Daha Fazla',
      liveMatches: 'Canlı Maçlar',
      noMatchesNow: 'Şu anda maç yok.',
      player: 'Oyuncu',
      player1: 'Oyuncu 1',
      player2: 'Oyuncu 2',
      bottomPlayer: 'Alt Oyuncu',
      topPlayer: 'Üst Oyuncu',
      queueLookingRated: 'Dereceli oyun aranıyor...',
      queueLookingUnrated: 'Derecesiz oyun aranıyor...',
      friends: 'Arkadaşlar',
      noFriendsYet: 'Henüz arkadaşın yok.',
    },
    banner: {
      slogan1Accent: 'Oyna',
      slogan1Text: 'Mangala',
      slogan2Accent: 'Nerede istersen',
      slogan2Text: 'oyna',
      slogan3Accent: 'Nasıl istersen',
      slogan3Text: 'oyna',
      play: 'Oyna',
      learn: 'Öğren',
      about: 'Hakkında',
      goHome: 'Ana Sayfaya Dön',
      playAsGuest: 'Misafir Olarak Oyna',
      rules: 'Kurallar',
      rule1: 'Her taraf 6 kuyudaki taşlara 4 taş ile başlar.',
      rule2: 'Taşlar saat yönünün tersine dağıtılır ve rakip hazinesi atlanır.',
      rule3: 'Son taşın kendi hazinene düşmesi sana bir tur daha kazandırır.',
      rule4: 'Son taş rakip kuyusunda kalıp sayıyı çift yaparsa o taşlar alınır.',
      rule5: 'Oyun, iki taraftan birinin kuyuları boşaldığında biter.',
    },
    playModal: {
      matchmaking: 'Eşleştirme',
      gameSetup: 'Oyun Kurulumu',
      searchingForGame: 'Oyun aranıyor',
      play: 'Oyna',
      cancelSearch: 'Aramayı iptal et',
      closePlaySetup: 'Oyun kurulumunu kapat',
      mode: 'Mod',
      matchMode: 'Maç modu',
      opponents: 'Rakipler',
      allowBots: 'Botlara İzin Ver',
      humansOnly: 'Sadece İnsanlar',
      humanOpponentsOnly: 'Sadece insan rakipler',
      botsAllowed: 'Botlara izin veriliyor',
      startSearch: 'Aramayı Başlat',
    },
    profile: {
      loadingProfile: 'Profil yükleniyor',
      fetchingPlayerDetails: 'Oyuncu bilgileri getiriliyor...',
      noSuchPlayerDescription: 'Bu kullanıcı adı hiçbir oyuncuyla eşleşmiyor.',
      memberSince: 'Üyelik tarihi',
      online: 'Çevrimiçi',
      offline: 'Çevrimdışı',
      rating: 'Puan',
      ratingHistory: 'Puan Geçmişi',
      noRatingHistory: 'Henüz puan geçmişi yok.',
      matchHistory: 'Maç Geçmişi',
      opponent: 'Rakip',
      result: 'Sonuç',
      resultWin: 'Kazandı',
      resultLoss: 'Kaybetti',
      resultDraw: 'Berabere',
      resultLive: 'Canlı',
      score: 'Puan',
      noMatchesSaved: 'Henüz kayıtlı maç yok.',
      seeMore: 'Daha Fazla',
      challenge: 'Meydan Oku',
      challengeTitle: 'Meydan Oku',
      challengeText: 'Maç türünü seç.',
      challengeError: 'Maç başlatılamadı.',
      loadFailed: 'Profil yüklenemedi.',
      ai: 'AI',
      addFriend: 'Arkadaş Ekle',
      removeFriend: 'Arkadaşlıktan Çıkar',
      requestPending: 'İstek Bekliyor',
    },
    profileGames: {
      loading: 'Maç geçmişi yükleniyor',
      fetching: 'Oyuncu maçları getiriliyor...',
      title: 'Maç Geçmişi',
      backToOverview: 'Genel Görünüme Dön',
      date: 'Tarih',
      loadFailed: 'Maç geçmişi yüklenemedi.',
      pages: 'Maç geçmişi sayfaları',
    },
    matchmaking: {
      targetNotBot: 'Bu kullanıcı bot değil.',
      userBusy: 'Bu kullanıcı şu anda başka bir maç oynuyor.',
      selfBusy: 'Zaten aktif bir maçınız var.',
    },
    account: {
      title: 'Hesap Ayarları',
      username: 'Kullanıcı adı',
      email: 'E-posta',
      currentPassword: 'Mevcut şifre',
      newPassword: 'Yeni şifre',
      confirmNewPassword: 'Yeni şifreyi onayla',
      pleaseLogInAgain: 'Hesabınızı güncellemek için lütfen tekrar giriş yapın.',
      enterPasswordFirst: 'Önce şifrenizi girin.',
      usernameRules:
        'Kullanıcı adı 3-15 karakter olmalı ve yalnızca harf, sayı, alt çizgi veya tire içermelidir.',
      changesSaved: 'Değişiklikler kaydedildi.',
      sessionExpired: 'Oturum süresi doldu. Lütfen yeniden giriş yapıp tekrar deneyin.',
      updateFailed: 'Hesap ayarları güncellenemedi.',
      saveChanges: 'Değişiklikleri Kaydet',
      saving: 'Kaydediliyor...',
      sending: 'Gönderiliyor...',
      resendVerification: 'Doğrulama E-postasını Tekrar Gönder',
      pendingVerification: 'Doğrulama Bekleyen E-posta:',
      verified: 'Doğrulandı',
      unverified: 'Doğrulanmadı',
      emailManagedByGoogle: 'E-posta Google tarafından yönetilmektedir.',
      googleSetPasswordInfo: 'Google ile giriş yaptınız. Gelecekte e-posta ve şifre ile giriş yapabilmek için buradan bir şifre belirleyebilirsiniz.',
    },
    leaderboard: {
      competition: 'Rekabet',
      title: 'Liderlik Tablosu',
      backToLobby: 'Lobiye Dön',
      loading: 'Liderlik tablosu yükleniyor...',
      loadFailed: 'Liderlik tablosu yüklenemedi.',
      noPlayers: 'Henüz oyuncu yok.',
      prev: 'Önceki',
      next: 'Sonraki',
      pages: 'Liderlik tablosu sayfaları',
      searchUsers: 'Oyuncu ara...',
    },
    learn: {
      title:
        'Mangala, iki kişiyle oynanan geleneksel bir Türk zekâ ve strateji oyunudur. Tahtada her oyuncunun önünde altışar küçük kuyu ve taşların toplandığı birer hazine bulunur. Oyun toplam 48 taşla oynanır.',
      intro:
        'Oyunun başında 12 küçük kuyunun her birine dörder taş yerleştirilir. Oyuncular kendi hazinelerinde en fazla taşı toplamayı hedefler. Oyun sonunda hazinesinde daha çok taş bulunan oyuncu kazanır. Aşağıda oyunun temel kuralları özetlenmiştir.',
      rule1Title: '1. Temel Kural',
      rule1Text:
        'Oyuncu kendi bölgesindeki istediği kuyudan taşları alır. Aldığı kuyuda bir taş bırakır ve elindeki kalan taşları saat yönünün tersine, yani sağa doğru, her kuyuya birer birer dağıtır. Son taş kendi hazinesine düşerse oyuncu bir kez daha oynar. Bir kuyuda yalnızca bir taş varsa, oyuncu sırası geldiğinde o taşı sağındaki kuyuya taşıyabilir. Hamlenin sonucunu elde kalan son taş belirler.',
      rule2Title: '2. Temel Kural',
      rule2Text:
        'Oyuncu kendi kuyularında taş dağıtmayı bitirdikten sonra elinde taş kalırsa rakibin bölgesindeki kuyulara taş bırakmaya devam eder. Son taş rakibin kuyusundaki taş sayısını çift yaparsa, oyuncu o kuyudaki tüm taşları alıp kendi hazinesine koyar. Sıra rakibe geçer.',
      rule3Title: '3. Temel Kural',
      rule3Text:
        'Oyuncunun elindeki son taş kendi bölgesindeki boş bir kuyuya düşerse ve tam karşı kuyuda rakibe ait taş varsa, oyuncu hem karşı kuyudaki taşları hem de kendi bıraktığı taşı alıp hazinesine ekler. Sıra rakibe geçer.',
      rule4Title: '4. Temel Kural',
      rule4Text:
        'Oyunculardan birinin bölgesindeki taşlar bittiğinde oyun sona erer. Kendi bölgesinde ilk taşsız kalan oyuncu, rakibin bölgesinde kalan tüm taşları da kazanır. Bu nedenle oyun son ana kadar önemini korur.',
      source: 'Kaynak: mangala.com.tr',
    },
    chat: {
      messages: 'Mesajlar',
      noFriendsYet: 'Henüz arkadaşın yok.',
      typeMessage: 'Bir mesaj yaz...',
      matchChat: 'Maç Sohbeti',
      noMessagesYet: 'Henüz mesaj yok.',
      sayHi: 'Rakibine merhaba de!',
      matchHasEnded: 'Maç sona erdi.',
      send: 'Gönder',
    },
  },
  en: {
    header: {
      backToGame: 'Back to Game',
      account: 'Account',
      language: 'Language',
      settings: 'Settings',
      accountSettings: 'Account Settings',
      logOut: 'Log out',
      english: 'English',
      turkish: 'Türkçe',
      openProfile: 'Open profile',
      goHome: 'Go home',
      notifications: 'Notifications',
      noNewNotifications: 'No new notifications',
      wantsToBeFriends: 'wants to be friends!',
      accept: 'Accept',
      reject: 'Reject',
      verifyEmailWarning: 'Please verify your email address. Your account will be deactivated if not verified within 24 hours of registration.',
    },
    common: {
      close: 'Close',
      rated: 'Rated',
      unrated: 'Unrated',
      cancel: 'Cancel',
      loadingMatch: 'Loading match',
      fetchingLatestState: 'Fetching the latest game state...',
      gameUnavailable: 'Game unavailable',
      noSuchPlayer: 'No such player',
      draw: 'Draw',
      or: 'OR',
      action: 'Action',
      watch: 'Watch',
      resign: 'Resign',
      globalNavigation: 'Global navigation',
      couldNotLoadMatch: 'Could not load match.',
    },
    auth: {
      usernameOrEmail: 'Username or Email',
      username: 'Username',
      email: 'Email',
      password: 'Password',
      confirmPassword: 'Confirm Password',
      rememberMe: 'Remember me',
      loggingIn: 'Logging in...',
      logIn: 'Log in',
      signUp: 'Sign Up',
      signingUp: 'Signing up...',
      playAsGuest: 'Play as Guest',
      continueWithGoogle: 'Continue with Google',
      backToLogin: 'Back to Login',
      loginMissing: 'Please enter your email/username and password.',
      loginFailed: 'Login failed. Please try again.',
      registerMissing: 'Please fill in username, email, and password.',
      usernameRules:
        'Username must be 3-15 characters and use only letters, numbers, underscores, or hyphens.',
      passwordLength: 'Password must be {min}-{max} characters long.',
      passwordsNoMatch: 'Passwords do not match.',
      registrationFailed: 'Registration failed. Please try again.',
      resendVerification: 'Resend Verification Email',
      emailVerification: 'Email Verification',
      verifyingEmail: 'Verifying your email, please wait...',
      emailVerifiedSuccess: 'Email verified successfully!',
      emailVerifiedError: 'Verification failed. The link may be expired.',
      noToken: 'No verification token provided.',
      verificationResent: 'Verification email resent successfully.',
      verificationResendFailed: 'Failed to resend verification email.',
      forgotPasswordDesc: "Enter your email address and we'll send you a link to reset your password.",
      forgotPasswordSuccess: 'If your email is registered, a password reset link has been sent.',
      forgotPasswordError: 'Failed to send password reset email.',
      sendResetLink: 'Send Reset Link',
      sendingResetLink: 'Sending...',
      invalidResetToken: 'Invalid or missing reset token.',
      bothPasswordsRequired: 'Both password fields are required.',
      passwordResetSuccess: 'Password has been reset successfully.',
      passwordResetError: 'Failed to reset password.',
      createNewPassword: 'Create New Password',
      newPassword: 'New Password',
      resettingPassword: 'Resetting...',
      resetPassword: 'Reset Password',
      redirecting: 'Redirecting...',
      forgotPasswordLink: 'Forgot Password?',
      completeProfileTitle: 'Complete Your Profile',
      completeProfileDesc: 'Please choose a username for yourself. You can change this later.',
      completeProfileSubmit: 'Save and Start',
      completingProfile: 'Saving...',
    },
    game: {
      youWin: 'You win',
      youLose: 'You lose',
      matchFinished: 'Match finished',
      matchResult: 'Match result',
      viewingSelectedPosition: 'Viewing the selected position.',
      viewingLatestPosition: 'Viewing the latest position.',
      startPosition: 'Viewing the start position.',
      matchEndsInDraw: 'The match ends in a draw.',
      wins: '{name} wins.',
      toMove: '{name} to move',
      visualStones: 'Visual Stones',
      moveAnimation: 'Move Animation',
      unavailable: 'Unavailable',
      on: 'On',
      off: 'Off',
      moveNumber: 'Move',
      restartMatch: 'Restart match',
      gameUnavailableDescription: 'This game is not available in the current session.',
      winsOnTime: '{name} wins on time.',
      sowingStones: '{name} is sowing stones...',
      collectsStonesAndWins: '{name} collects more stones and wins.',
      playsAgain: '{name} landed in the store and plays again.',
      capturedStones: '{activeName} captured {captured} stones. {nextName} is up next.',
      winsByResignation: '{name} wins by resignation.',
      botOpponent: 'Bot opponent',
    },
    home: {
      welcome: 'Welcome {name}!',
      guestHeroTitle: 'Play Mangala online.',
      learnTrain: 'Learn & Train',
      resumeCurrentGame: 'Resume Current Game',
      play: 'Play',
      practiceBoard: 'Practice Board',
      lobbySidePanel: 'Lobby side panel',
      availablePlayers: 'Available Players',
      noAvailablePlayers: 'No available players right now.',
      leaderboard: 'Leaderboard',
      showMore: 'Show More',
      liveMatches: 'Live Matches',
      noMatchesNow: 'No matches right now.',
      player: 'Player',
      player1: 'Player 1',
      player2: 'Player 2',
      bottomPlayer: 'Bottom Player',
      topPlayer: 'Top Player',
      queueLookingRated: 'Looking for a rated game...',
      queueLookingUnrated: 'Looking for an unrated game...',
      friends: 'Friends',
      noFriendsYet: 'You have no friends yet.',
    },
    banner: {
      slogan1Accent: 'Play',
      slogan1Text: 'Mangala',
      slogan2Accent: 'Wherever',
      slogan2Text: 'you like',
      slogan3Accent: 'However',
      slogan3Text: 'you want',
      play: 'Play',
      learn: 'Learn',
      about: 'About',
      goHome: 'Go Home',
      playAsGuest: 'Play as Guest',
      rules: 'Rules',
      rule1: 'Each side begins with 4 stones in all 6 pits.',
      rule2: 'Stones are distributed counter-clockwise and the opponent store is skipped.',
      rule3: 'Landing your last stone in your own store grants another turn.',
      rule4: 'If the last stone lands in an opponent pit and makes it even, those stones are captured.',
      rule5: 'The game ends when either player side becomes empty.',
    },
    playModal: {
      matchmaking: 'Matchmaking',
      gameSetup: 'Game Setup',
      searchingForGame: 'Searching for a game',
      play: 'Play',
      cancelSearch: 'Cancel search',
      closePlaySetup: 'Close play setup',
      mode: 'Mode',
      matchMode: 'Match mode',
      opponents: 'Opponents',
      allowBots: 'Allow Bots',
      humansOnly: 'Humans Only',
      humanOpponentsOnly: 'Human opponents only',
      botsAllowed: 'Bots allowed',
      startSearch: 'Start Search',
    },
    profile: {
      loadingProfile: 'Loading profile',
      fetchingPlayerDetails: 'Fetching player details...',
      noSuchPlayerDescription: "This username doesn't match any player.",
      memberSince: 'Member since',
      online: 'Online',
      offline: 'Offline',
      rating: 'Rating',
      ratingHistory: 'Rating History',
      noRatingHistory: 'No rating history yet.',
      matchHistory: 'Match History',
      opponent: 'Opponent',
      result: 'Result',
      resultWin: 'Win',
      resultLoss: 'Loss',
      resultDraw: 'Draw',
      resultLive: 'Live',
      score: 'Rating',
      noMatchesSaved: 'No matches saved yet.',
      seeMore: 'See More',
      challenge: 'Challenge',
      challengeTitle: 'Challenge',
      challengeText: 'Choose the match type.',
      challengeError: 'Could not start the match.',
      loadFailed: 'Could not load profile.',
      ai: 'AI',
      addFriend: 'Add Friend',
      removeFriend: 'Remove Friend',
      requestPending: 'Request Pending',
    },
    profileGames: {
      loading: 'Loading match history',
      fetching: 'Fetching player matches...',
      title: 'Match History',
      backToOverview: 'Back to Overview',
      date: 'Date',
      loadFailed: 'Could not load match history.',
      pages: 'Match history pages',
    },
    matchmaking: {
      targetNotBot: 'This user is not a bot.',
      userBusy: 'This user is currently playing another match.',
      selfBusy: 'You already have an active match.',
    },
    account: {
      title: 'Account Settings',
      username: 'Username',
      email: 'Email',
      currentPassword: 'Current Password',
      newPassword: 'New Password',
      confirmNewPassword: 'Confirm New Password',
      pleaseLogInAgain: 'Please log in again to update your account.',
      enterPasswordFirst: 'Enter your password first.',
      usernameRules:
        'Username must be 3-15 characters and use only letters, numbers, underscores, or hyphens.',
      changesSaved: 'Changes saved.',
      sessionExpired: 'Session expired. Please log in again, then retry.',
      updateFailed: 'Could not update account settings.',
      saveChanges: 'Save Changes',
      saving: 'Saving...',
      sending: 'Sending...',
      resendVerification: 'Resend Verification Email',
      pendingVerification: 'Pending Verification Email:',
      verified: 'Verified',
      unverified: 'Unverified',
      emailManagedByGoogle: 'Email is managed by Google.',
      googleSetPasswordInfo: 'You signed in with Google. You can set a password here to enable email/password login in the future.',
    },
    leaderboard: {
      competition: 'Competition',
      title: 'Leaderboard',
      backToLobby: 'Back to Lobby',
      loading: 'Loading leaderboard...',
      loadFailed: 'Could not load leaderboard.',
      noPlayers: 'No players yet.',
      prev: 'Prev',
      next: 'Next',
      pages: 'Leaderboard pages',
      searchUsers: 'Search users...',
    },
    learn: {
      title:
        'Mangala is a traditional Turkish game of intelligence and strategy played by two people. The board has six small pits in front of each player and a treasury where stones are collected. The game is played with 48 stones.',
      intro:
        'At the beginning, four stones are placed in each of the 12 small pits. Players aim to collect the most stones in their treasury. The player with more stones in their treasury at the end wins the game. The main rules are summarized below.',
      rule1Title: '1. Basic Rule',
      rule1Text:
        'The player chooses any pit in their own territory, takes its stones, leaves one stone there, and distributes the rest counterclockwise, one by one, into the following pits. If the last stone lands in the player’s own treasury, that player moves again. If a pit contains only one stone, the player may move that stone to the pit on its right. The final stone in hand determines the result of the move.',
      rule2Title: '2. Basic Rule',
      rule2Text:
        'If the player still has stones after distributing in their own territory, they continue into the opponent’s pits. If the last stone makes the number of stones in an opponent pit even, the player captures all stones in that pit and places them into their own treasury. The turn then passes to the opponent.',
      rule3Title: '3. Basic Rule',
      rule3Text:
        'If the last stone in hand lands in an empty pit in the player’s own territory and the directly opposite pit contains opponent stones, the player captures both the opposite stones and the stone just placed, adding them to the treasury. The turn then passes to the opponent.',
      rule4Title: '4. Basic Rule',
      rule4Text:
        'The game ends when one player’s side runs out of stones. The player whose side becomes empty first also collects all stones remaining on the opponent’s side. For this reason, the game remains intense until the very end.',
      source: 'Source: mangala.com.tr',
    },
    chat: {
      messages: 'Messages',
      noFriendsYet: 'You have no friends yet.',
      typeMessage: 'Type a message...',
      matchChat: 'Match Chat',
      noMessagesYet: 'No messages yet.',
      sayHi: 'Say hi to your opponent!',
      matchHasEnded: 'Match has ended.',
      send: 'Send',
    },
  },
}
